...
def trajectory(windows, logMbh, sma, ecc, incl, spin, phi_r0, phi_theta0, phi_phi0, dt, adot=0.0, edot=0.0):
    ...
    if secular:
        ...
        # Build the gap-augmented grid in NumPy (cheap; nwin is small) and then move
        # it to the active backend.  ``windows`` is always a NumPy array, and this
        # avoids backend-specific behaviour of linspace / boolean masking with 0-d
        # device-array endpoints on the GPU path.
        grid_parts, mask_parts = [], []
        prev_stop = None
        for start, stop in windows:
            wg = np.arange(start, stop, dt)
            if prev_stop is not None:
                fill = np.linspace(prev_stop, wg[0], N_GAP + 2)[1:-1]   # gap interior only
                grid_parts.append(fill)
                mask_parts.append(np.zeros(fill.shape[0], dtype=bool))
            grid_parts.append(wg)
            mask_parts.append(np.ones(wg.shape[0], dtype=bool))
            prev_stop = wg[-1]
        t_sec = xp.asarray(np.concatenate(grid_parts))
        real_mask = xp.asarray(np.concatenate(mask_parts))
    else:
    ...
    # Orbital period evaluated at t = 0: a fixed reference for the disk precession
    # period (P_d = P_d_frac * P_orb), independent of the orbiter's later drift.
    # (Column 0 is window 0's first sample -- always a real point, never gap-fill.)
    P_orb = 2*xp.pi / Omega_phi[:, 0] * Omega_t[:, 0] * M

    # Drop the inter-window gap sub-samples (if any): the crossing search only
    # uses the real observation-window samples; the gap samples existed solely to
    # make the phase integral accurate across the gaps.  In the static case
    # real_mask is None and the grid is already gap-free.
...
def disk_normal(t_sec, r, theta_d, P_d_sec, phi_d, r_warp):
    ...
    """
    # maximum() guards the rare r<0 from the truncated PN series (only reachable
    # for pathological sampled orbits) and is backend-safe on numpy and cupy;
    # r_warp=0 still yields gamma_w=0 exactly.
...
def residuals(timings, windows, errs, sma, e, incl, phi_r0, phi_theta0, phi_phi0,
    ...
    # Optional disk parameters: broadcast to (nparams,) and convert to physical
    # units.  Their defaults (OPTIONAL_DEFAULTS) leave the disk aligned, static
    # and flat, recovering the original residuals exactly.
