# QPE-FIT with agnostic secular timing evolution
...
**Optional agnostic secular evolution:** add `"Pdot": value` to `params.json` to inject a timing-level secular period derivative into mock data. `Pdot` has units of s/s. If omitted, `qpe-gen` uses `Pdot = 0` and returns the original crossing-time model.
...
The sampler also supports one optional agnostic secular-evolution parameter:
- `Pdot`: timing-level period derivative in s/s. If `Pdot` is included in `priors.json`, `qpe-fit` samples it and applies the secular timing drift inside the likelihood. If it is omitted, `Pdot` is fixed to zero and the likelihood is unchanged from the original QPE-FIT model.
...
cat > priors.json << EOF
{
    ...
    "Pdot": {"bounds": [-1e-5, 1e-4], "wrapped": false}
}
EOF
