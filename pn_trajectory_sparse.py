...
def trajectory(windows, logMbh, sma, ecc, incl, spin, phi_r0, phi_theta0, phi_phi0, n=10000):
	...
	t_g = cp.concatenate([cp.linspace(start, stop, n) for start, stop in windows]) / (4.926580927874239e-06 * 10**cp.array(logMbh)[:, None])
	...
	return t, r, cartesian(r, cos_theta, phi), lambd

# def residuals(timings, windows, errs, sma, e, incl, a, logMbh, theta_obs, phi_r0, phi_theta0, phi_phi0, theta_d, phi_d, P_d):
def residuals(timings, windows, errs, sma, e, incl, a, logMbh, theta_obs, theta_d, phi_d, P_d):
	t_g = 4.926580927874239e-06 * 10**cp.array(logMbh)
	t, r, (x, y, z), _ = trajectory(windows, logMbh, sma, e, incl, a, np.zeros_like(a), np.zeros_like(a), np.zeros_like(a))
	phi_d = cp.array(phi_d)
	theta_d = cp.radians(cp.array(theta_d))
	t = t * t_g[:, None]
	P_d = cp.array(P_d) * t_g
	theta_obs = cp.radians(cp.array(theta_obs))
	...
	shapiro_delay_all = -2 * t_g[:, None] * np.log(r * (1 + dot_product))
	geometric_delay_all = -r * dot_product * t_g[:, None]
	D_t = n_crs_x * x + n_crs_y * y + n_crs_z * z
	crossings_mask = (D_t[:, :-1] * D_t[:, 1:]) < 0
	num_valid = cp.sum(crossings_mask, axis=1)
	max_num_crossings = int(cp.max(num_valid).get())
	crossings = cp.where(crossings_mask, t[:, :-1] + shapiro_delay_all[:, :-1] + geometric_delay_all[:, :-1], cp.nan)
