                    ...
                    for P_d in P_ds:
                        for i, window in enumerate(windows):
                            if i > 2:
                                ...
                                dt = 10
                            preamble = f"#!/bin/bash\n#SBATCH --output=slurmfiles/sma={sma}_e={e}_a={spin}_Mbh={Mbh}_theta_d={theta_d}_Pd={P_d}_windows={i}.out\n#SBATCH --error=slurmfiles/sma={sma}_e={e}_a={spin}_Mbh={Mbh}_theta_d={theta_d}_Pd={P_d}_windows={i}.err\n#SBATCH --job-name=timing_sampler\n#SBATCH --partition=sched_mit_kburdge_r8\n#SBATCH --gres=gpu:1\n\n"
                            timing_file = f'timings_sma={sma}_e={e}_a={spin}_Mbh={Mbh}_theta_d={theta_d}_Pd={P_d}_windows={i}.dat'
                            window_file = f'windows_sma={sma}_e={e}_a={spin}_Mbh={Mbh}_theta_d={theta_d}_Pd={P_d}_windows={i}.dat'
                            outfile = f'h5files/sma={sma}_e={e}_a={spin}_Mbh={Mbh}_theta_d={theta_d}_Pd={P_d}_windows={i}.h5'
                            os.system(f'echo "{preamble}" > sample.sh')
                            cmd = f'python generate_timings.py {sma} {e} 60 {spin} {Mbh} {theta_d} {P_d} {window} {timing_file} {window_file}'
